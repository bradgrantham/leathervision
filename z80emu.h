...
void Z80_FETCH_BYTE(ADDR address, DATA& x)
{
    ...
    for(auto it = boards.begin(); it != boards.end(); it++)
        (*it)->memory_read((address) & 0xffff, b);
    x = b;
}
...
template <typename ADDR, typename DATA>
void Z80_READ_BYTE(ADDR address, DATA& x)
{
    /* last one wins. */
    unsigned char b = 0x00;
    for(auto it = boards.begin(); it != boards.end(); it++)
...
template <typename ADDR, typename DATA>
void Z80_WRITE_BYTE(ADDR address, DATA x)
{
    /* they all win. */
    unsigned char b = x;
    for(auto it = boards.begin(); it != boards.end(); it++)
        (*it)->memory_write(address & 0xffff, b);
}

template <typename ADDR, typename DATA>
void Z80_READ_WORD(ADDR address, DATA& x)
{
    int l, h;
    Z80_FETCH_BYTE(address, l);
    Z80_FETCH_BYTE(address + 1, h);
...
template <typename ADDR, typename DATA>
void Z80_WRITE_WORD(ADDR address, DATA x)
{
    Z80_WRITE_BYTE(address, x & 0xff);
    Z80_WRITE_BYTE(address + 1, x >> 8);
}

template <typename ADDR, typename DATA>
void Z80_INPUT_BYTE(ADDR port, DATA& x)
{
    /* SystemCall(state); */
    /* last one wins. */
    unsigned char b = 0x00;
...
template <typename ADDR, typename DATA>
void Z80_OUTPUT_BYTE(ADDR port, DATA x)
{
    /* number_cycles = 0; */
